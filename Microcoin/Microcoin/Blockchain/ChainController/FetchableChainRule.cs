...
namespace Microcoin.Microcoin.Blockchain.ChainController
{
    public class FetchableChainRule : IFetchableChainRule
    {
        public bool IsPossibleChainUpgrade(AbstractChain chain, Microcoin.Blockchain.Block.Block block)
        {
            ...
            //int minimumComplexityToSwitch = Convert.ToInt32(currentTailBlock.MiningBlockInfo.ChainComplexity * 1.0);
            if (currentTailBlock.MiningBlockInfo.ChainComplexity < block.MiningBlockInfo.ChainComplexity)
                return true;
            else
                return false;
        }
